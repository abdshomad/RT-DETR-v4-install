#!/usr/bin/env python3
"""
Train RT-DETRv4-M (Medium) model on chicken detection dataset.

Usage:
    python train-rt-detr-v4-m.py [--use-amp] [--seed SEED] [--resume PATH] [--tune PATH] [--test-only]
"""
...
def main():
    """Main training function for RT-DETRv4-M."""
    parser = argparse.ArgumentParser(
        description="Train RT-DETRv4-M model on chicken detection dataset",
    ...
    config_path = (configs.PROJECT_ROOT / configs.CONFIG_PATHS["m"]).resolve()
    ...
    output_dir = args.output_dir or str((configs.PROJECT_ROOT / configs.OUTPUT_DIRS["m"]).resolve())
    cmd.extend(["--update", f"output_dir={output_dir}"])
    
    # Print configuration
    print("\n" + "=" * 60)
    print("RT-DETRv4-M Training Configuration")
    print("=" * 60)
